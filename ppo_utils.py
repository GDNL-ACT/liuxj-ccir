...
def get_rewards_from_server(server_url: str, messages: list[str]) -> list["torch.Tensor"]:
    ...
    import re
    keyword_file = "data/ccir_generation_ppo.json"
        # 读取关键词配置
    with open(keyword_file, "r", encoding="utf-8") as f:
        keyword_data = json.load(f)
    query2keywords = {
        item["instruction"].strip(): item["keywords"] for item in keyword_data
    }

    def get_user_query(message: str) -> str:
        user_turns = re.findall(r"<\|im_start\|>user\n(.*?)<\|im_end\|>", message, re.DOTALL)
        return user_turns[-1].strip() if user_turns else ""

    def get_assistant_answer(message: str) -> str:
        assistant_turns = re.findall(r"<\|im_start\|>assistant\n(.*?)<\|im_end\|>", message, re.DOTALL)
        return assistant_turns[-1].strip() if assistant_turns else ""

    rewards = []
    for msg in messages:
        query = get_user_query(msg)
        answer = get_assistant_answer(msg)
