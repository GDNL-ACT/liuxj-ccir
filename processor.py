...
class Processor:
    def __init__(self, process_type: str, batch_size: int = 4, model_path: str = None):
        ...
        self.process_methods = {
            "current_question_eval": self._current_question_eval,
            "prefix_question_eval": self._prefix_question_eval,
            "rewrite_question_eval": self._rewrite_question_eval,
            "rewrite_question_train": self._rewrite_question_train
        }
        ...
        self.rewrite_builder = PromptBuilder(
            system_prompt=(
                ...
                "5. 若原问题信息不足，应从历史问题中提取明确信息补全，不得随意扩展；\n"
    ...
    def run(self, original_data_path: str, output_path: str):
        ...
        if self.process_type == "rewrite_question_train":
            with open(original_data_path, 'r', encoding='utf-8') as f:
                raw_list = json.load(f)
            processed = method(raw_list)
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(processed, f, ensure_ascii=False, indent=2)
            logging.info(f"Processed training data to {output_path}")
            return

        if self.process_type != "rewrite_question_eval":
            with open(original_data_path, "r", encoding="utf-8") as f:
                raw_list = json.load(f)
        else:
            with open(original_data_path, 'r', encoding='utf-8') as f:
    ...
    def _rewrite_question_train(self, data_list):
    ...
    def _rewrite_question_eval(self, data_list):
        self._ensure_model_loaded()
        messages_list = []
        message_to_turn_ref = []
        ...
        for i in tqdm(range(0, len(messages_list), self.batch_size), desc="Rewriting queries for evaluate"):
            batch_messages = messages_list[i:i + self.batch_size]
            rewritten_outputs = self._generate_with_prompt_builder(batch_messages)
    ...
    def _current_question_eval(self, data_list):
        for data in data_list:
    ...
    def _prefix_question_eval(self, data_list):
        for data in data_list:
            prefix = ""
            for conversation in data.get("conversation", []):
...
if __name__ == "__main__":
    ...
    processor.run("/home/liuxj25/LawLLM/CCIR/data/dataset1.json", "output/rewritten_queries.json")
